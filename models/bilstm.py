...
class BiLstmModel(BaseModel):
    def __init__(self, config, embedded_words=None, labels=None, vocab_size=None, word_vectors=None):
        ...
        with tf.variable_scope(self.config['model_name']):
            ...
            if labels is not None:
                # 给定的情况下，使用给定的，否则使用默认占位符
                self.labels = labels

            # 构建模型
            tf.logging.info("**** Start initialization Bi-LSTM model. ****")
            self.build_model()
            # 初始化保存模型的saver对象
            tf.logging.info("**** Start Bi-LSTM model saver. ****")
            self.init_saver()

    def build_model(self):
        if self.embedded_words is None:
            # 词嵌入层
            ...
            # 直接使用输入的词向量(一个批次的)
            embedded_words = self.embedded_words
